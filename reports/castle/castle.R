# Script to prep data for Castle Forecast PDF
...
# Get Local Forecast for Castle
#load(file="output/pdf_castle.Rda")
if(file.info("graphs/48h_forecast_Castle_wind.png")$mtime<=(file.info("cache/ec_gemregional/ec_gemregional.nc")$mtime+60) | !exists("local.data"))
{
  message("[Generating Local Forecast for Castle...]")
  local.cells<-getCells.GEMRegional(type="resorts")
  local<-local.cells[which(local.cells$resort=="Castle"),]
  local.data<-plot.GEMRegional.local(poi="Castle",type="resorts",limit.data=FALSE)
...
# Save Data for sheet
save(local.data.sum,local.data,local.cells,file="output/pdf_castle.Rda")

# Knit PDF document
message("[Generating PDF...]")
require(knitr)
setwd("reports/castle/")
knit(input="castle.Rnw",output="castle.tex",quiet=TRUE)
system("pdflatex castle.tex")
setwd("../../")
