# Script to prep data for Revelstoke Forecast PDF
...
# Get Local Forecast for Revelstoke
#load(file="output/pdf_revelstoke.Rda")
if(file.info("graphs/48h_forecast_Revelstoke_wind.png")$mtime<=(file.info("cache/ec_gemregional/ec_gemregional.nc")$mtime+60) | !exists("local.data"))
{
  message("[Generating Local Forecast for Revelstoke...]")
  local.cells<-getCells.GEMRegional(type="resorts")
  local<-local.cells[which(local.cells$resort=="Revelstoke"),]
  local.data<-plot.GEMRegional.local(poi="Revelstoke",type="resorts",limit.data=FALSE)
...
# Save Data for sheet
save(local.data.sum,local.data,local.cells,file="output/pdf_revelstoke.Rda")

# Knit PDF document
message("[Generating PDF...]")
require(knitr)
setwd("reports/revelstoke/")
knit(input="revelstoke.Rnw",output="revelstoke.tex",quiet=TRUE)
system("pdflatex revelstoke.tex")
setwd("../../")
