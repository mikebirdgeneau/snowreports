# Script to prep data for Emerald Lake Forecast PDF
...
# Get Local Forecast for Emerald Lake
#load(file="output/pdf_emerald_lake.Rda")
if(file.info("graphs/48h_forecast_Emerald_Lake_wind.png")$mtime<=(file.info("cache/ec_gemregional/ec_gemregional.nc")$mtime+60) | !exists("local.data"))
{
  message("[Generating Local Forecast for Emerald Lake...]")
  local.cells<-getCells.GEMRegional(type="backcountry")
  local<-local.cells[which(local.cells$resort=="Emerald Lake"),]
  local.data<-plot.GEMRegional.local(poi="Emerald Lake",type="backcountry",limit.data=FALSE)
  local.data$logo.path<-get.weather.logo(local.data,type="backcountry")
  local.sun<-fetch.sunsriset(poi=local.data$resort[1],type="backcountry",dateTime=local.data$date)
...
# Save Data for sheet
save(local.data.sum,local.data,local.cells,file="output/pdf_emerald_lake.Rda")

# Knit PDF document
message("[Generating PDF...]")
require(knitr)
setwd("reports/emerald_lake/")
knit(input="emerald_lake.Rnw",output="emerald_lake.tex",quiet=TRUE)
system("pdflatex emerald_lake.tex")
setwd("../../")
