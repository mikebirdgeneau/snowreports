# Script to prep data for Fernie Forecast PDF
...
# Get Local Forecast for Fernie
#load(file="output/pdf_fernie.Rda")
if(file.info("graphs/48h_forecast_Fernie_wind.png")$mtime<=(file.info("cache/ec_gemregional/ec_gemregional.nc")$mtime+60) | !exists("local.data"))
{
  message("[Generating Local Forecast for Fernie...]")
  local.cells<-getCells.GEMRegional(type="resorts")
  local<-local.cells[which(local.cells$resort=="Fernie"),]
  local.data<-plot.GEMRegional.local(poi="Fernie",type="resorts",limit.data=FALSE)
...
# Save Data for sheet
save(local.data.sum,local.data,local.cells,file="output/pdf_fernie.Rda")

# Knit PDF document
message("[Generating PDF...]")
require(knitr)
setwd("reports/fernie/")
knit(input="fernie.Rnw",output="fernie.tex",quiet=TRUE)
system("pdflatex fernie.tex")
setwd("../../")
