# Script to prep data for Canmore Forecast PDF
...
# Get Local Forecast for Canmore
#load(file="output/pdf_canmore.Rda")
if(file.info("graphs/48h_forecast_Canmore_wind.png")$mtime<=(file.info("cache/ec_gemregional/ec_gemregional.nc")$mtime+60) | !exists("local.data"))
{
  message("[Generating Local Forecast for Canmore...]")
  local.cells<-getCells.GEMRegional(type="towns")
  local<-local.cells[which(local.cells$resort=="Canmore"),]
  local.data<-plot.GEMRegional.local(poi="Canmore",type="towns",limit.data=FALSE)
...
local.data.sum<-local.data[!is.na(period),list(
  ...
  sunrise=max(sunrise),
  sunset=min(sunset),
  periods=sum(0*elev+1)),by=c("resort","period")]
...
local.data.sum$icon<-get.weather.logo(local.data.sum,type = "towns") # Note this doesn't copy the data table, so adds cloudy, etc. columns
...
# Save Data for sheet
save(local.data.sum,local.data,local.cells,file="output/pdf_canmore.Rda")

# Knit PDF document
message("[Generating PDF...]")
require(knitr)
setwd("reports/canmore/")
knit(input="canmore.Rnw",output="canmore.tex",quiet=TRUE)
system("pdflatex canmore.tex")
setwd("../../")
