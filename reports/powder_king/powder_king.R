# Script to prep data for Powder King Forecast PDF
...
# Get Local Forecast for Powder King
load(file="output/pdf_powder_king.Rda")
if(file.info("graphs/48h_forecast_Powder_King_wind.png")$mtime<=(file.info("cache/ec_gemregional/ec_gemregional.nc")$mtime+60) | !exists("local.data"))
{
  message("[Generating Local Forecast for Powder King...]")
  local.cells<-getCells.GEMRegional(type="resorts")
  local<-local.cells[which(local.cells$resort=="Powder King"),]
  local.data<-plot.GEMRegional.local(poi="Powder King",type="resorts",limit.data=FALSE)
...
# Save Data for sheet
save(local.data.sum,local.data,local.cells,file="output/pdf_powder_king.Rda")

# Knit PDF document
message("[Generating PDF...]")
require(knitr)
setwd("reports/powder_king/")
knit(input="powder_king.Rnw",output="powder_king.tex",quiet=TRUE)
system("pdflatex powder_king.tex")
setwd("../../")
