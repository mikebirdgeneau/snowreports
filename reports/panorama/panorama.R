# Script to prep data for Panorama Forecast PDF
...
# Get Local Forecast for Panorama
#load(file="output/pdf_panorama.Rda")
if(file.info("graphs/48h_forecast_Panorama_wind.png")$mtime<=(file.info("cache/ec_gemregional/ec_gemregional.nc")$mtime+60) | !exists("local.data"))
{
  message("[Generating Local Forecast for Panorama...]")
  local.cells<-getCells.GEMRegional(type="resorts")
  local<-local.cells[which(local.cells$resort=="Panorama"),]
  local.data<-plot.GEMRegional.local(poi="Panorama",type="resorts",limit.data=FALSE)
...
# Save Data for sheet
save(local.data.sum,local.data,local.cells,file="output/pdf_panorama.Rda")

# Knit PDF document
message("[Generating PDF...]")
require(knitr)
setwd("reports/panorama/")
knit(input="panorama.Rnw",output="panorama.tex",quiet=TRUE)
system("pdflatex panorama.tex")
setwd("../../")
