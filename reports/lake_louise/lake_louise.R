# Script to prep data for Lake Louise Forecast PDF
...
# Get Local Forecast for Lake Louise
load(file="output/pdf_lake_louise.Rda")
if(file.info("graphs/48h_forecast_Lake_Louise_wind.png")$mtime<=(file.info("cache/ec_gemregional/ec_gemregional.nc")$mtime+60) | !exists("local.data"))
{
  message("[Generating Local Forecast for Lake Louise...]")
  local.cells<-getCells.GEMRegional(type="resorts")
  local<-local.cells[which(local.cells$resort=="Lake Louise"),]
  local.data<-plot.GEMRegional.local(poi="Lake Louise",type="resorts",limit.data=FALSE)
  local.data$logo.path<-get.weather.logo(local.data)
  local.sun<-fetch.sunsriset(poi=local.data$resort[1],type="resorts",dateTime=local.data$date)
  local.data$sunrise<-local.sun$sunrise
  local.data$sunset<-local.sun$sunset
  #local.sun<-fetch.sunsriset(poi=local.data$resort[1],type="resorts",dateTime=Sys.time())
} else {
...
local.data.sum<-local.data[!is.na(period),list(
  ...
  temp.trend=mean(c(diff(temperature),0)),
                                 ...
                                 ifelse(
                                   ...
                                   ifelse(
                                     ...
                                     ifelse(
                                       ...
                                       ))),"C\\newline Avg. wind ",winddir.comp," ",wind,"km/h","\\newline Humidity: ",round(relhum,0),"\\%",
                                 "\\newline Pressure ",press," kPa",press.trend.txt),]
local.data.sum

# Save Data for sheet
save(local.data.sum,local.data,local.cells,file="output/pdf_lake_louise.Rda")

# Knit PDF document
message("[Generating PDF...]")
require(knitr)
setwd("reports/lake_louise/")
knit(input="lake_louise.Rnw",output="lake_louise.tex",quiet=TRUE)
#system("pdflatex lake_louise.tex")

#if(file.info("lake_louise.pdf")$mtime>(Sys.time()-300))
#{
  system(paste0("pdflatex lake_louise.tex; sendEmail -f snowbot@snow-reports.ca -t snow-reports.ca_lake_louise@lists.snow-reports.ca -u \"Lake Louise Weather Report\" -m \"Lake Louise Weather Forecast",format(Sys.time(),"%A %b %d, %Y %H:%M"),"\" -a /home/snowreports/snow-reports.ca/reports/lake_louise/lake_louise.pdf"))
#} else {
#  system("sendEmail -f snowbot@snow-reports.ca -t mike@snow-reports.ca -u \"Lake Louise Weather Report - Update Failed!\" -m \"Check PDF Generation. Updated file not detected.\"")
#}
