...
# Get Snowfall Data
kicking_horse<-read_html("http://kickinghorseresort.com/conditions/snow-report/")

updated<-(kicking_horse %>% html_nodes("div.zn_text_box.eluidcf95568e.zn_text_box-light.element-scheme--light h4 div") %>% html_text(trim=TRUE))[1]

updated<-parse_date_time(updated[1],orders = c("mdy T","mdy R","md R"),truncated=3,tz = "America/Edmonton")

test <- c(kicking_horse %>% html_nodes(".rcr-element-box-fl16") %>% html_text(trim=TRUE),kicking_horse %>% html_nodes(".rcr-element-box-fl16") %>% html_text(trim=TRUE))
