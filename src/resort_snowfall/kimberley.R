...
# Get Snowfall Data
kimberley<-read_html("http://skikimberley.com/conditions/snow-report/")

#eluid080c2b22 > div.zn_section_size.container.zn-section-height--auto.zn-section-content_algn--top > div.row.zn_columns_container.zn_content > div.eluida44cd96f.col-md-9.col-sm-9.zn_sortable_content.zn_content.znColumnElement > div.zn_text_box.eluidab6e1416.zn_text_box-light.element-scheme--light > h4 > div

updated<-(kimberley %>% html_nodes("div.zn_text_box.eluidab6e1416.zn_text_box-light.element-scheme--light h4 div") %>% html_text(trim=TRUE))[1]

updated<-parse_date_time(updated[1],orders = c("mdy T","mdy R","md R"),truncated=3,tz = "America/Edmonton")

test <- c(kimberley %>% html_nodes(".rcr-element-box-fl16") %>% html_text(trim=TRUE),kimberley %>% html_nodes(".rcr-element-box-fl16") %>% html_text(trim=TRUE))
