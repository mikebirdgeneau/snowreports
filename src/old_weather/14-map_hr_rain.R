...
rain <- ncvar_get(model,"RPRATE_surface")[,,]
rain <- aperm(rain,c(1,2,3))
plot.data<-rain

rm(model0,model,temp,rain)
gc()
...
ani.loop<-function(){
  ...
  {    
    ...
    CairoPNG(file=paste0("graphs/24h_forecast_rain_tmp",file.no,".png"),width=640, height=480)
    ...
    p1.sm=spplot(test2,c("rain"),names.attr=c("Rain Accumulation"),
		col.regions=colorRampPalette(c("white", "#C5E9FF", "#3288BD", "#66C2A5", "#ABDDA4", "#E6F598","#FFFFBF", "#FEE08B", "#FDAE61", "#F46D43", "#D53E4F", "#9E0142")),
          ...
          colorkey=list(labels=list(at=c(1,2.5,5,7.5,10,15,20,25,30,35,40,45,50,60,70,80),labels=c(1,2.5,5,7.5,10,15,20,25,30,35,40,45,50,60,70,80))),                
                 main=paste0("Cumulative Rainfall (mm) at ",date.fancy[i]),
                 xlab="Snow-Reports.ca - Forecast Data: Environment Canada (Regional HRDPS West)\n Map: GeoGratis © Department of Natural Resources Canada. All rights reserved.",
    ...
    dev.off()
    if(i==24){
      CairoPNG(file=paste0("graphs/24h_forecast_rain",file.no,".png"),width=800, height=600)
      print(p1.sm)
      dev.off()
    }
...
max<-dim(plot.data)[3]
for(i in (max+1):(max+3))
{
   system(paste0("cp graphs/24h_forecast_rain_tmp",max,".png graphs/24h_forecast_rain_tmp",i,".png"))
}

rm(list=ls())
gc()
system("convert -delay 25 -loop 50 graphs/24h_forecast_rain_tmp*.png graphs/24h_forecast_rain_tmp.gif")
system("mv graphs/24h_forecast_rain_tmp.gif graphs/24h_forecast_rain.gif")
#system("rm graphs/24h_forecast_rain_tmp*.gif")
