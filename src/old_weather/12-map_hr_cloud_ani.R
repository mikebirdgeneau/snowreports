...
cloud <- ncvar_get(model,"TCDC_surface")[,,]/100
cloud <- aperm(cloud,c(1,2,3))
plot.data<-cloud

rm(model0,model,cloud)
gc()
...
ani.loop<-function(){
  ...
  {    
    ...
    CairoPNG(file=paste0("graphs/24h_forecast_cloud_tmp",file.no,".png"),width=640, height=480)
    ...
    p1.sm=spplot(test2,c("cloud"),names.attr=c("Snow Accumulation"),
		 col.regions=colorRampPalette(rev(brewer.pal(9,name="PuBu"))), 
		#col.regions=colorRampPalette(c("#111111","#333333","#666666","#999999","#BBBBBB","#DDDDDD","#FFFFFF")),
          at=c(-0.01,seq(0,1,by=0.01),1.01),
                 main=paste0("Forecast Cloud Cover at ",date.fancy[i]),
...
{
   system(paste0("cp graphs/24h_forecast_cloud_tmp",max,".png graphs/24h_forecast_cloud_tmp",i,".png"))
}
...
system("convert -delay 35 -loop 50 graphs/24h_forecast_cloud_tmp*.png graphs/24h_forecast_cloud_tmp.gif")
system("mv graphs/24h_forecast_cloud_tmp.gif graphs/24h_forecast_cloud.gif")
