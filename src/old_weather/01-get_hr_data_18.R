...
date=format(as.POSIXct(Sys.time()-6*3600, tz=format(Sys.time(),"%Z")),"%Y%m%d",tz="UTC")
date.fancy=format(as.POSIXct(Sys.time()-6*3600, tz=format(Sys.time(),"%Z")),"%Y-%m-%d",tz="UTC")

# Prefix
prefix="http://dd.weatheroffice.gc.ca/model_hrdps/west/grib2/18/"
suffix=".grib2"
