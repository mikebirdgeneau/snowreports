...
model0 <- nc_open("results/naefs_000.nc")
model <- nc_open("results/naefs.nc",readunlim=FALSE)
...
resorts<-data.frame(c("Apex",-119.901258,49.383206),
                    ...
                    c("Stanley Mitchell Hut",-116.5643,51.5256),
                    c("Elizabeth Parker Hut",-116.3444,51.3556),
...
grid=SpatialGrid(GridTopology(bbox(coords)[,1],c(1,1),c(360,181)))
rm(x,y)
...
resort.cells$row<-ceiling(resort.cells$cellno/360)
resort.cells$col<-360-(resort.cells$row*360-resort.cells$cellno)

# Get data by resort, over 24 hr period.
plot.data<-data.frame()
for(i in 1:nrow(resort.cells)){
  tmp.date=as.POSIXct(format(as.POSIXct.numeric(ncvar_get(model,"time",start=c(1),count=c(19)),tz="UTC",origin="1970-01-01"),tz="America/Edmonton"))
  tmp.temperature<-ncvar_get(model,"TMP_2maboveground",start=c(resort.cells$col[i],resort.cells$row[i],1),count=c(1,1,19))-273.15
  tmp.sprate<-ncvar_get(model,"SPRATE_surface",start=c(resort.cells$col[i],resort.cells$row[i],1),count=c(1,1,19))
  #tmp.rprate<-ncvar_get(model,"RPRATE_surface",start=c(resort.cells$col[i],resort.cells$row[i],1),count=c(1,1,19))
  #tmp.apcp<-ncvar_get(model,"APCP_surface",start=c(resort.cells$col[i],resort.cells$row[i],1),count=c(1,1,19))
  tmp.clouds<-ncvar_get(model,"TCDC_localleveltype2000",start=c(resort.cells$col[i],resort.cells$row[i],1),count=c(1,1,19))/100
  #tmp.wind<-ncvar_get(model,"WIND_10maboveground",start=c(resort.cells$col[i],resort.cells$row[i],1),count=c(1,1,19))*3.6
  #tmp.winddir<-ncvar_get(model,"WDIR_10maboveground",start=c(resort.cells$col[i],resort.cells$row[i],1),count=c(1,1,19))
  #tmp.press<-ncvar_get(model,"PRES_surface",start=c(resort.cells$col[i],resort.cells$row[i],1),count=c(1,1,19))/1E3
  tmp<-data.frame(
  ...
  rm(tmp.date,tmp.sprate,tmp.temperature,tmp.clouds)
...
for(i in 1:length(resortlist))
{
  ...
  pcp.data<-subset(resort.data,select=c(resort,date,snow.acc))
  setnames(pcp.data,c("snow.acc"),c("Cum. Snow"))
  ...
  file.loc=str_replace_all(paste0("graphs/7d_forecast_",resortlist[i],"_tmp.png")," ","_")
  ...
  grid.arrange(p1,p2,nrow=2,main = paste0("\nWeather Forecast for 7-Day Period at ",resort.data$resort[1]," - Snow-Reports.ca"))
  dev.off()
}
...
for(i in 1:length(resortlist)){
  file.loc=str_replace_all(paste0("graphs/7d_forecast_",resortlist[i],"_tmp.png")," ","_")
  file.dest=str_replace_all(paste0("graphs/7d_forecast_",resortlist[i],".png")," ","_")
