...
ani.loop<-function(){
  ...
  {    
    ...
    CairoPNG(file=paste0("graphs/24h_forecast_snow_tmp",file.no,".png"),width=640, height=480)
    ...
    p1.sm=spplot(test2,c("snow"),names.attr=c("Snow Accumulation"),
		col.regions=colorRampPalette(c("white", "#C5E9FF", "#3288BD", "#66C2A5", "#ABDDA4", "#E6F598","#FFFFBF", "#FEE08B", "#FDAE61", "#F46D43", "#D53E4F", "#9E0142")),
          ...
          colorkey=list(labels=list(at=c(1,2.5,5,7.5,10,15,20,25,30,35,40,45,50,60,70,80),labels=c(1,2.5,5,7.5,10,15,20,25,30,35,40,45,50,60,70,80))),                
                 main=paste0("Cumulative Snowfall (cm) at ",date.fancy[i]),
...
{
   system(paste0("cp graphs/24h_forecast_snow_tmp",max,".png graphs/24h_forecast_snow_tmp",i,".png"))
}

rm(list=ls())
gc()
system("convert -delay 25 -loop 50 graphs/24h_forecast_snow_tmp*.png graphs/24h_forecast_snow_tmp.gif")
system("mv graphs/24h_forecast_snow_tmp.gif graphs/24h_forecast_snow.gif")
