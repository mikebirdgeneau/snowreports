...
p2=spplot(test2,c("temp.min"),names.attr=c("Forecast Low (°C)"),
          ...
          main=paste0("24h Forecast Low Temperature (°C) for ",date.fancy),
          scales=list(draw=FALSE),
          xlim=c(plotlim@bbox[1],plotlim@bbox[3]),
          ylim=c(plotlim@bbox[2],plotlim@bbox[4]),
          sp.layout=list(c('sp.points',cities,col="black",pch=21,cex=0.8,lwd=1,fill="grey"),
...
png(filename="graphs/24h_forecast_low_tmp.png",width=700,height=465,units="px",res=72,type="cairo")
print(p2)
dev.off()
rm(p2)
gc()
...
p3=spplot(test2,c("temp.max"),names.attr=c("Forecast High (C)"),
         col.regions=rgb.palette,
          at=c(seq(floor(min(test2@data$temp.min)/5)*5,ceiling(max(test2@data$temp.max)/5)*5,by=2.5)),
          main=paste0("24h Forecast High (°C) for ",date.fancy),
         scales=list(draw=FALSE),
         xlim=c(plotlim@bbox[1],plotlim@bbox[3]),
         ylim=c(plotlim@bbox[2],plotlim@bbox[4]),
          sp.layout=list(c('sp.points',cities,col="black",pch=21,cex=0.8,lwd=1,fill="grey"),
...
png(filename="graphs/24h_forecast_high_tmp.png",width=700,height=465,units="px",res=72,type="cairo")
print(p3)
dev.off()
rm(p3)
gc()
...
system("mv graphs/24h_forecast_low_tmp.png graphs/24h_forecast_low.png")
system("mv graphs/24h_forecast_high_tmp.png graphs/24h_forecast_high.png")
