...
coords<-spTransform(coords,CRS("+proj=latlong +a=6371229 +b=6371229 +no_defs"))
...
test2<-flipVertical(SpatialGridDataFrame(grid,plot.data))
...
canada_borders <- spTransform(canada_borders, CRS("+proj=latlong +a=6371229 +b=6371229 +no_defs"))
canada_borders <- as(canada_borders, "SpatialLinesDataFrame")
roads<-readOGR(dsn="shapefiles/roads.shp",layer="roads")
roads <- spTransform(roads, CRS("+proj=latlong +a=6371229 +b=6371229 +no_defs"))
...
cities<-spTransform(cities,CRS('+proj=latlong +a=6371229 +b=6371229 +no_defs'))
...
resorts<-spTransform(resorts,CRS('+proj=latlong +a=6371229 +b=6371229 +no_defs'))
...
plotlim<-spTransform(plotlim,CRS('+proj=latlong +a=6371229 +b=6371229 +no_defs'))
rm(x,y,coords,flipVertical)

p1=spplot(test3,c("var1.pred"),names.attr=c("Snow Accumulation"),
          ...
          main=paste0("7-day NAEFS Snowfall Forecast (cm) for ",date.fancy),
