# plot
plot.GEMRegional.local<-function(poi="Lake Louise",type="resorts",limit.data=FALSE,plotOutput=TRUE)
{
  ...
  if(plotOutput==TRUE){
    ...
    p1<-ggplot()+
      ...
      ggtitle(paste0("Forecast Precipitation for ",resort.data$resort[1]," at ",resort.data$elev[1],"m"))+
    ...
    p2<-ggplot()+
      ...
      #xlab("")+
      xlab(paste0("Forecast Time (",format(Sys.time(),"%Z",tz="America/Edmonton"),") - Source: Environment Canada"))+
      ylab("Temperature\n(Celsius)")+
      ylim(min(-10,resort.data$temperature),max(0,resort.data$temperature))+
      ggtitle(paste0("Forecast Temperatures for ",resort.data$resort[1]," at ",resort.data$elev[1],"m"))+
    ...
    png(filename=paste0(path,"graphs/48h_forecast_",str_replace_all(resort.data$resort[1]," ","_"),"_precipitation.png"),width=600,height=200)
    print(p1)
    dev.off()
    png(filename=paste0(path,"graphs/48h_forecast_",str_replace_all(resort.data$resort[1]," ","_"),"_temperature.png"),width=600,height=200)
    print(p2)
    dev.off()
    png(filename=paste0(path,"graphs/48h_forecast_",str_replace_all(resort.data$resort[1]," ","_"),"_wind.png"),width=600,height=150)
    grid.arrange(p3,p4,p5,p6,nrow=1)
    dev.off()
    
    png(filename=paste0(path,"graphs/48h_forecast_",str_replace_all(resort.data$resort[1]," ","_"),".png"),width=790,height=700)
    grid.arrange(p1,p2,arrangeGrob(p3,p4,p5,p6,nrow=1),nrow=3)
    dev.off()
    
    save(p1,p2,p3,p4,p5,p6,file=paste0("cache/plots/ec_gemregional_",str_replace_all(str_replace_all(poi," ","_"),"\\.",""),"_plots.Rda"))
  }
