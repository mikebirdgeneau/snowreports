...
plot.GEMGlobal.precip<-function(type="snow",output=TRUE,file.suffix="",width=1200,height=600,res=72,limit.hr=NA,min.hr=0)
{
  graphics.off()
  if(is.na(limit.hr)){
    limit.hr=120
  ...
  title=switch(var,
               SPRATE_surface = paste0(limit.day," day Forecast Snowfall"),
               RPRATE_surface = paste0(limit.day," day Forecast Rain (mm)"))
  
  filename=NULL
  if(output){
    filename=switch(var,
                    SPRATE_surface = paste0(ifelse(limit.hr<100,"0",""),limit.hr,"h_forecastGEMGlobal_snow"),
                    RPRATE_surface = paste0(ifelse(limit.hr<100,"0",""),limit.hr,"h_forecastGEMGlobal_rain"),
    )
  }
  
  #var="TMP_2maboveground" #Debugging
  ...
  variable <- ncvar_get(model, var,start=c(1,500,min.hr.limit),count=c(564,-1,limit.hr/3-min.hr.limit))#[,,1:limit.hr/3]
  ...
  if(!exists("shpCanBorders") | !exists("shpCanRoads") | !exists("shpUSBorders"))
  {
