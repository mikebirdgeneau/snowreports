...
plot.NAM.precip<-function(type="snow",output=FALSE,file.suffix="",width=700,height=465,res=72,min.hr=0,max.hr=NA)
{  
  ...
  title=switch(var,
               CSNOW_surface = paste0(min.hr,"-",max.hr,"h Forecast Snowfall"),
               CRAIN_surface = paste0(min.hr,"-",max.hr,"h Forecast Rain (mm)"))
  
  filename=NULL
  if(output){
    filename=switch(var,
                    CSNOW_surface = paste0(min.hr,"-",max.hr,"h_forecastNAM_snow"),
                    CRAIN_surface = paste0(min.hr,"-",max.hr,"h_forecastNAM_rain"),
    )
  }
  ...
  model <- nc_open(paste0(path,"cache/noaa_nam/noaa_nam.nc"))
  date.fancy=format(as.POSIXct.numeric(min(ncvar_get(model,"time")),tz="GMT",origin="1970-01-01"),"%Y-%m-%d %H:%M %Z",tz="America/Edmonton")
  ...
  hrs<-as.vector(as.numeric((time-min(time))/3600))
  which(hrs<=max.hr & hrs>min.hr,arr.ind=FALSE)
  var1<-"APCP_surface"
  ...
  variable <- ncvar_get(model, var1)[,,which(hrs<=max.hr & ifelse(min.hr==0,hrs>=min.hr,hrs>min.hr),arr.ind=FALSE)]
  variable2 <- ncvar_get(model, var)[,,which(hrs<=max.hr & ifelse(min.hr==0,hrs>=min.hr,hrs>min.hr),arr.ind=FALSE)]
  
  variable <- aperm(variable,c(1,2,3))
  variable2 <- aperm(variable2,c(1,2,3))
  
  variable <- variable2 * variable
  
  variable=as.vector(apply(variable,MARGIN=c(1,2),FUN=sum))
  
  plot.data<-data.table(variable=variable)
  suppressWarnings(rm(variable,variable2,model,model0))
  ...
  coords<-spTransform(coords,CRS(proj.NAM))
  ...
  if(!exists("loadShapefiles")){
    source("lib/load_shapefiles.R")
  }
  loadShapefiles(crs=proj.NAM)
  
  # Add annotations for major cities / towns & Resorts
  if(!exists("poi.cities"))
  {
    source("lib/points_of_interest.R")
  }
  cities<-spTransform(poi.cities,CRS(proj.NAM))
  resorts<-spTransform(poi.resorts,CRS(proj.NAM))
  
  rgb.palette=colorRampPalette(rev(brewer.pal(n=9,name="Spectral")))
  plotlim=data.frame(x=c(-131,-110),y=c(44,58))
  coordinates(plotlim)<-c('x','y')
  proj4string(plotlim)='+proj=longlat +ellps=GRS80 +towgs84'
  plotlim<-spTransform(plotlim,CRS(proj.NAM))
  rm(x,y,coords)
